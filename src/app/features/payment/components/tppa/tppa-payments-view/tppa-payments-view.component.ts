...
@Component({
  selector: 'app-tppa-payments-view',
  templateUrl: './tppa-payments-view.component.html',
  styleUrls: ['./tppa-payments-view.component.scss']
})
export class TppaPaymentsViewComponent implements OnInit, OnDestroy, AfterViewInit {

  // OnDestroy 
  componentDestroyed$: Subject<boolean> = new Subject();
  // API - View Component Data
  request_item: any = {
    selected_transaction: [], history: [], selected_user: [], accounts: [], notifications: [], applications: [],
  ...
  @ViewChild(TppaPaymentsActionsComponent, { static: false }) TppaPaymentsActionsComponent: TppaPaymentsActionsComponent;
  ...
  ngAfterViewInit() {
    // this.TppaPaymentsActionsComponent.resetApiErrorModal('fromParent');
    // this.TppaPaymentsActionsComponent.resetSelectedActionModal('fromParent');
  }
  ...
  getRequestHistory(requestData) {
    ...
    this._tppaPayments.getTppaPaymentsHistoryDetails(requestData).pipe(takeUntil(this.componentDestroyed$)).subscribe((data: any) => {
      ...
      this.request_item.history = data;
      console.log('getRequestHistory this.request_item.history', this.request_item.history);
  ...
  selectTransaction(requestData) {
    ...
    console.log('selectAccount -> this.request_item.selected_transaction ', this.request_item.selected_transaction);
  }
