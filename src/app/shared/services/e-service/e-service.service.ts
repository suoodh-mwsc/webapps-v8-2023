...
export class EServiceService {
    ...
    return this._http.post<any>(`${environment.staffAppApiConfig.api_url}Utility/ePortalUser/Find`, body, httpOptions)
      .pipe(map(res => {
        ...
        catchError((error: any) => {
          console.log('eService -> error', error);
  ...
  // POST XAPI/Account/Notifications
  postePortalUserNotifications(data) {
    const httpOptions = this._httpHeaderService.prepareOptions();
    const body = JSON.stringify(data);
    return this._http.post<any>(`${environment.staffAppApiConfig.api_url}XAPI/Account/Notifications`, body, httpOptions)
      .pipe(map(res => {
        // this.leaveRequestsSubject.next(res);
        return res;
      },
